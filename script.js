...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function recalcAll() {
        ...
        // compute balance due: grand total minus any amount paid
        const amountPaid = amountPaidEl ? parseNumber(amountPaidEl.value) : 0;
    ...
    // --- Invoice / Due date dynamic behavior ---
    ...
    function updateDueDate() {
        ...
        let invDate = parseISODate(invoiceDateInput && invoiceDateInput.value);
        if (!invDate) invDate = new Date();

        // determine offset based on terms
        let offsetDays = 0;
        if (termsEl) {
            const t = termsEl.value;
            if (t === 'net-7') offsetDays = 7;
            else if (t === 'net-15') offsetDays = 15;
            else if (t === 'net-30') offsetDays = 30;
            else offsetDays = 0; // due-on-receipt
        }

        const due = new Date(invDate.getTime());
        due.setDate(due.getDate() + offsetDays);

        if (invoiceDateInput && !invoiceDateInput.value) invoiceDateInput.value = toISODateString(invDate);
        if (dueDateInput) dueDateInput.value = toISODateString(due);
    }

    // init invoice date to today and wire up listeners
    const invoiceDateEl = document.getElementById('invoice-date');
    if (invoiceDateEl) {
        // set default to today if not set in markup
        if (!invoiceDateEl.value) invoiceDateEl.value = toISODateString(new Date());
        invoiceDateEl.addEventListener('change', function () {
            updateDueDate();
            // recalc totals in case someone wants side-effects tied to date change
            recalcAll();
        });
    }
    ...
    const termsEl = document.getElementById('terms');
    if (termsEl) {
        // set default selected value to current (if markup left it default, keep it)
        termsEl.addEventListener('change', function () {
            updateDueDate();
        });
    }

    // initial due date compute
    updateDueDate();

    // --- Make company and party addresses editable and persistent ---
    ...
    function saveEditable(id) {
        try {
            ...
            if (!el) return;
            localStorage.setItem('invoice.' + id, el.innerHTML);
        } catch (e) { /* ignore storage errors */ }
    }

    function loadEditable(id, fallbackHtml) {
        try {
    ...
    ['company-name', 'company-address', 'bill-name', 'bill-addr'].forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        // load saved content if any
        const fallback = el.innerHTML;
        const html = loadEditable(id, fallback);
        el.innerHTML = html;

        const saver = debounce(function () { saveEditable(id); }, 400);
        // save on input and blur (works across browsers)
        el.addEventListener('input', saver);
        el.addEventListener('blur', function () { saveEditable(id); });
    });
    ...
    // amount paid input should trigger recalculation too
    const amountPaidInput = document.getElementById('amount-paid');
    if (amountPaidInput) amountPaidInput.addEventListener('input', recalcAll);
    recalcAll();
